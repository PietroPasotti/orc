...
def _set_task_status_and_comment(
    task_name: str, status: str, author: str, comment: str, work_dir: Path
) -> None:
    board_file = work_dir / "board.yaml"
    if not board_file.exists():
        return
...
def main() -> None:
    ...
    if task_name:
        _set_task_status_and_comment(task_name, "rejected", args.agent_id, args.message, work_dir)
    else:
        print(  # noqa: T201
