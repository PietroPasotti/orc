...
def _orc_dir(project_root: Path) -> Path:
    """Return the .orc directory for a project."""
    return project_root / ".orc"
...
class TestBootstrap:
    ...
    def test_creates_vision_readme(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        runner.invoke(m.app, ["bootstrap"])
        readme = _orc_dir(tmp_path) / "vision" / "README.md"
    ...
    def test_creates_work_readme(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        runner.invoke(m.app, ["bootstrap"])
        readme = _orc_dir(tmp_path) / "work" / "README.md"
        assert readme.exists()
        assert len(readme.read_text()) > 0
    ...
    def test_creates_gitignore_with_work_and_vision(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        runner.invoke(m.app, ["bootstrap"])
        gitignore = tmp_path / ".orc" / ".gitignore"
